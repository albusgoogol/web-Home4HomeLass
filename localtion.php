<?php
    ...
    $sql2 = "SELECT * FROM native
      ...
      WHERE native.hl_code = '".$_SESSION['hl']."' ";
  ...
  $sql3 = "SELECT * FROM address
      ...
      WHERE address.hl_code = '".$_SESSION['hl']."' ";
    ...
    $val = $_GET['val'];
          ...
          if ($data=='geography') { 
             ...
             while($rst=mysql_fetch_array($query)){ 
                  if($rst['GEO_ID']==$rs2['GEO_ID'])
                      $selgeo='selected';
                  else $selgeo=''; 
                  echo "<option value='$rst[GEO_ID]' $selgeo >$rst[GEO_NAME]</option>" ;
              ...
              while($rst2=mysql_fetch_array($query2)){ 
                    if($rst2['prov_code']==$rs2['prov_code'])
                      $selprov='selected';
                    else $selprov='';
                   echo "<option value=\"$rst2[prov_code]\" $selprov >$rst2[prov_name]</option>" ;
              }
         } 
         else if ($data=='amphur') {
              ...
              $sql3="SELECT * from amphur 
                    WHERE prov_code = '$val' 
                    ORDER BY AMPHUR_NAME";  
              $query3=mysql_query($sql3); 
              while($rst3=mysql_fetch_array($query3)){ 
                if($rst3['am_code']==$rs2['am_code'])
                    $selam='selected';
                else $selam='';                          
              echo "<option value=\"$rst3[am_code]\" $selam >$rst3[AMPHUR_NAME]</option> " ;
              }
              ...
              while($rst4=mysql_fetch_array($query4)){ 
                  if($rst4['dis_code']==$rs2['dis_code'])
                    $seldis='selected';
                  else $seldis=''; 
                   echo "<option value=\"$rst4[dis_code]\" $seldis >$rst4[DISTRICT_NAME]</option> \n" ;
              }
         }
