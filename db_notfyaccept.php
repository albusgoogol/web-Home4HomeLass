...
/*if (empty($_POST['cause']) || empty($_POST['status']) || empty($_POST['tservice']) ){
  ...
  if($objResultcheck == ""){
	...
	$new_native = mysql_result(mysql_query("Select Max(substr(n_code,-6))+1 as MaxID from  native"),0,"MaxID");
		...
		$strSQL1 = "INSERT INTO homeless (hl_code,hl_id,hl_fname,hl_lname,hl_sex,hl_bday,age_code,lastjob_code,ad_code,n_code) 
		VALUES ('$hl_idnew','".$_POST["id"]."','".$_POST["fname"]."','".$_POST["lname"]."','".$_POST["sex"]."','".$_POST["bday"]."','".$_POST["age"]."',
		...
		$strSQL4 = "INSERT INTO homelessdetail (hl_code,mem_code,hldetail_date,hldetail_period,hldetail_status,hl_location) 
		VALUES ('$hl_idnew','$mcode','".$_POST["day"]."','".$_POST["period"]."','จากการรับแจ้ง','".$_POST["location"]."')";
		$objQuery4 = mysql_query($strSQL4);

		//echo $strSQL4;

		$upstatushin = "UPDATE homelessinform SET
 					hlin_status = 'ได้รับความช่วยเหลือเบื้องต้น'
 					WHERE homelessinform.hlin_id = '$hlin_id' ";
 		$objstatushin = mysql_query($upstatushin);

 		$upimghl = "UPDATE imagehomeless SET 
 					hl_code = '$hl_idnew' 
 					WHERE imagehomeless.hlin_id = '$hlin_id' ";
 		$objimghl = mysql_query($upimghl);
		...
		echo "<meta http-equiv='Content-Type' content='text/html; charset=UTF8' />";
		echo "<script type='text/javascript'>alert('บันทึกการเข้ารับบริการจากการรับแจ้งสำเร็จ!')</script>";
		echo "<script>window.location='notification.php'</script>";//เมื่อแก้ไขข้อมูลเรียบร้อยแล้วสั่งให้ลิงค์ไปตำแหน่งที่เราต้องการ
	}else {
		echo "<meta http-equiv='Content-Type' content='text/html; charset=UTF8' />";
