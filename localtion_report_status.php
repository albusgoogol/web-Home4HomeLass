...
$sql = "SELECT status_name as monthse,COUNT('hl_code') AS 'status'  FROM statusdetail 
        INNER JOIN statusphysical ON statusphysical.status_id = statusdetail.status_id
        INNER JOIN homeless ON homeless.hl_code = statusdetail.hl_code 
        INNER JOIN homelessdetail ON homelessdetail.hl_code = homeless.hl_code
        ".$where."
        GROUP BY statusphysical.status_name ";
//}
